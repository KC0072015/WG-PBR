...
WG1_PRIVKEY=$(awk '/^\[Interface\]/,/^\[Peer\]/' "$WG1_CONF" | grep '^PrivateKey' | awk '{print $3}')
WG1_ADDRESS=$(awk '/^\[Interface\]/,/^\[Peer\]/' "$WG1_CONF" | grep '^Address' | awk '{print $3}')
WG1_PUBKEY=$(awk '/^\[Peer\]/,0' "$WG1_CONF" | grep '^PublicKey' | awk '{print $3}')
WG1_ENDPOINT=$(awk '/^\[Peer\]/,0' "$WG1_CONF" | grep '^Endpoint' | awk '{print $3}')
WG1_KEEPALIVE=$(awk '/^\[Peer\]/,0' "$WG1_CONF" | grep '^PersistentKeepalive' | awk '{print $3}')
...
WG1_ENDPOINT_IP=$(dig +short "$WG1_ENDPOINT_HOST" | grep -E '^[0-9]+\.' | head -1)
...
# Append one ipset directive per domain from domains.txt
...
log "=== Stack is up ==="
log "  WireGuard server : wg0 ($WG_SERVER_IP) — listen :51820"
...
# Keep the container alive; exit if dnsmasq dies
wait $DNSMASQ_PID
